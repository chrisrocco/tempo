 ...
 * to `defineActivities` and used nowhere else. Rename the file and the check stops
 * applying; reach for `payments` directly and lint fails.
 *
 * The worker in `spec/integration/define_activities.spec.ts` passes `workflows` and
...
const act = defineActivities(payments, {retry: {maximumAttempts: 1}});
