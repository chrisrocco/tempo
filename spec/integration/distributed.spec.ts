...
function waitForLine(proc: ChildProcess, re: RegExp, timeoutMs = 20000): Promise<RegExpMatchArray> {
  return new Promise((resolve, reject) => {
    let out = '';
    let err = '';
    const to = setTimeout(() => reject(new Error(`timeout for ${re}\n[out]${out}\n[err]${err}`)), timeoutMs);
    const onOut = (d: Buffer) => {
      out += d.toString();
      const m = out.match(re);
      if (m) { cleanup(); resolve(m); }
    };
    const onErr = (d: Buffer) => { err += d.toString(); };
    const onExit = (code: number | null) => { cleanup(); reject(new Error(`exited ${code}\n[out]${out}\n[err]${err}`)); };
    const cleanup = () => {
      clearTimeout(to);
...
async function pollUntil<T>(fn: () => Promise<T | undefined>, timeoutMs = 5000): Promise<T> {
...
async function spawnServer(env: Record<string, string> = {}): Promise<{ url: string; proc: ChildProcess }> {
  const proc = spawnMain('bin/server-main.ts', env);
  const m = await waitForLine(proc, /LISTENING (\d+)/);
  return { url: `http://127.0.0.1:${m[1]}`, proc };
}

const remote = (url: string, opts?: RemoteServiceOptions) => createRemoteService(url, opts);
...
describe('distributed — real server process over RPC', () => {
  it('runs a workflow across separate server + workflow-worker + activity-worker processes', async () => {
    ...
    const wf = spawnMain('bin/workflow-worker-main.ts', { SERVER_URL: url, WORKER_MODULE: FIXTURE });
    const act = spawnMain('bin/activity-worker-main.ts', { SERVER_URL: url, WORKER_MODULE: FIXTURE });
    try {
      ...
      await expectAsync(service.getResult(workflowId)).toBeResolvedTo('hi world');
    } finally {
      await kill(wf);
      await kill(act);
      await kill(server);
  ...
  it('redelivers an activity after its lease expires, running it at-least-once', async () => {
    const { url, proc: server } = await spawnServer({ ACTIVITY_LEASE_MS: '60' });
    const service = remote(url);
    ...
    activityRegistry.set('work', () => { runs += 1; return 'done'; });
    ...
    try {
      ...
      wfLoop = runWorkflowWorker(service, createWorkflowWorker(workflowRegistry));
      ...
      await service.completeActivityTask(second.token, { ok: true, result: 'done' });
