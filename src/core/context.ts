...
// user workflow code without an explicit parameter (see doc 02, the ALS caveat).
...
interface BlockedCondition { fn: () => boolean; resolve: () => void; reject: (e: unknown) => void }
...
export interface WorkflowContext {
  ...
  done: boolean; result: unknown;
  failed: boolean; failure: unknown;
...
export function createContext(
  ...
  return {
    args, events, idx: 0, seq: 0, condSeq: 0,
