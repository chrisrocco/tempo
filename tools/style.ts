 ...
 *
 * 2. **Top-level await, anywhere.** This repo is CommonJS (`tsconfig.json`
 *    says why), and CommonJS has no top-level await — it is
 ...
 *
 * There was a fourth: unqualified browser globals in `dashboard/app/`, which had
 * to live here rather than in `tools/conventions.ts` because only a checker can
 * tell `window.history` from the local named `history` next to it. It went with
 * the dashboard, and with it the only reason this file built more than one
 * program — the engine and a browser cannot share a `lib`.
 */
 ...
 * because it was `['bin/']` until the repo became CommonJS, and the empty
    ...
    const visit = (node: ts.Node): void => {
      ...
      if (bansTopLevelAwait && ts.isAwaitExpression(node) && isTopLevel(node))
        violations.push({
          ...
          message:
            'top-level await does not exist in CommonJS (TS1378), and this repo is CommonJS — use `void fn().then(…)`',
        });
...
/**
 * Build a program from the repo's tsconfig, so the rules see what `tsc` sees.
 *
 * One config, and it took a parameter until the dashboard went: the engine and a
 * browser cannot share a program, because they are checked under different libs.
 * Everything left here is Node.
 */
