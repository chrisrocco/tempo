...
| **spec**                | When a schedule fires. An **interval spec** is a period in milliseconds, boundaries aligned to the epoch; a **calendar spec** is a wall-clock rule — minute/hour/day fields in an IANA timezone, DST policy in `schedule/calendar.ts`. |
