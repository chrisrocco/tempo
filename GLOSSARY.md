...
| **checkpoint** | What a running attempt reports with `heartbeat(…)` — a **register, not a log**: one slot per attempt, overwritten by the next beat, discarded with the attempt, never a history event. Must be _complete_ every time, because only a sample of beats is sent. `src/worker/activity_context.ts`. |
