...
| **awaiting**             | Free-form JSON a `condition()` declares about what would release it, carried on the parked state and stamped on `conditionParked`. Opaque to the engine; conventions like `kind: 'approval'` belong to `patterns/` helpers and the UIs that honor them. Capped at `MAX_AWAITING_BYTES`. `src/core/condition.ts`. |
...
| **completion event**    | An event that settles a command and resolves what was waiting — `activityCompleted`, `activityCancelled`, `timerFired`, `childFailed`. Carries the `seq` it completes.                                                                                                                                                              |
...
| **checkpoint** | What a running attempt reports with `heartbeat(…)` — a **register, not a log**: one slot per attempt, overwritten by the next beat, never a history event of its own — the last one is carried onto the activity's terminal event when the attempt settles. Capped at `MAX_CHECKPOINT_BYTES`. Must be _complete_ every time, because only a sample of beats is sent. `src/worker/activity_context.ts`. |
...
| **spec**                | When a schedule fires. An **interval spec** is a period in milliseconds, boundaries aligned to the epoch; a **calendar spec** is a wall-clock rule — minute/hour/day fields in an IANA timezone, DST policy in `src/libraries/walltime/`, the internally-owned library the engine treats as third-party. |
